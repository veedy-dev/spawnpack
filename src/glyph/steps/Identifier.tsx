...
export function StepIdentifier({ value, suggested, onChange, onNext, onBack }: Props) {
    const [error, setError] = useState("");
    const suggestion = useMemo(() => {
        const normalized = sanitizeIdentifier(suggested).replace(/-/g, "_");
        return normalized || "cool_addon";
    }, [suggested]);
    ...
    return (
        <Box style={{ flexGrow: 1, flexDirection: "column", gap: 1 }}>
            <Text style={{ bold: true, color: "cyan" }}>Addon identifier</Text>
            <Text style={{ dim: true }}>Internal slug used in generated pack and folder names.</Text>
            <Spacer size={1} />
            <Input
                value={value}
                onChange={nextValue => {
